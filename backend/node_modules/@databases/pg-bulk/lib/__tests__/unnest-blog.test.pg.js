...
test('insert', async () => {
  ...
  `);
    expect(await db.query((0, pg_1.sql) `SELECT * FROM ${table} ORDER BY email ASC`))
...
test('select', async () => {
    ...
    expect(await db.query((0, pg_1.sql) `
      SELECT * FROM ${table}
...
test('update', async () => {
  ...
  `);
    expect(await db.query((0, pg_1.sql) `SELECT * FROM ${table} ORDER BY email ASC`)).toMatchInlineSnapshot(`Array []`);
});
//# sourceMappingURL=unnest-blog.test.pg.js.map