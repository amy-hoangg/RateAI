...
test('create schema', async () => {
    await db.query((0, pg_1.sql) `CREATE SCHEMA typed_queries_tests`);
...
test('attributes', () => {
    ...
    expect(users(db).tableId.format(pg_1.pgFormat).text).toBe('"typed_queries_tests"."users"');
    expect(photos(db).tableName).toBe('photos');
    expect(photos(db).tableId.format(pg_1.pgFormat).text).toBe('"typed_queries_tests"."photos"');
});
test('create users', async () => {
  ...
  `);
    ...
    const updated = await photos(db).update({ cdn_url: 'http://example.com/1' }, {
        metadata: { rating: 5 },
    });
  ...
  `);
    const [forbes2, john] = await users(db).insertOrUpdate(['screen_name'], { screen_name: 'Forbes', bio: 'Author of @databases' }, { screen_name: 'John', bio: 'Just a random name' });
  ...
  `);
    const insertOrIgnoreResults = await users(db).insertOrIgnore({ screen_name: 'John', bio: 'Updated bio' }, { screen_name: 'Martin', bio: 'Updated bio' });
    ...
    expect((_b = (await users(db).find().orderByDesc('screen_name').first())) === null || _b === void 0 ? void 0 : _b.screen_name).toMatchInlineSnapshot(`"Martin"`);
...
test('uses tableId in custom queries', async () => {
    const result = await db.query((0, pg_1.sql) `
...
test('use a default connection', async () => {
    ...
    const { users: mockUsers } = (0, __1.default)({
        schemaName: 'typed_queries_tests',
        defaultConnection: {
    ...
    const { users: unconnectedUsers } = (0, __1.default)({
        schemaName: 'typed_queries_tests',
    });
    expect(() => unconnectedUsers(undefined)).toThrowErrorMatchingInlineSnapshot(`"You must either provide a \\"defaultConnection\\" to pg-typed, or specify a connection when accessing the table."`);
});
test('insert or update', async () => {
    ...
    function exp(user, { created, updated }) {
        expect({
    ...
    const [userA2, userB2] = await users(db).insertOrUpdate({ onConflict: [`screen_name`], set: ['screen_name', 'updated_at'] }, {
        screen_name: USER_A,
        created_at: time2,
        updated_at: time2,
    }, {
    ...
    const [userA3, userB3] = await users(db).insertOrUpdate({ onConflict: [`screen_name`], doNotSet: ['created_at'] }, {
        screen_name: USER_A,
        created_at: time3,
        updated_at: time3,
    }, {
