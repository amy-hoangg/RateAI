...
test('create schema', async () => {
    await db.query((0, pg_1.sql) `CREATE SCHEMA typed_queries_bulk_insert`);
...
test('create multiple users per statement using non-bulk API', async () => {
    const [forbes, ellie, john] = await users(db).insert({ screen_name: 'Forbes' }, { screen_name: 'Ellie', age: 10 }, { screen_name: 'John', bio: 'Hello world' });
  ...
  `);
    const [forbes2, martin, ellie2, john2] = await users(db).insertOrIgnore({ screen_name: 'Forbes' }, { screen_name: 'Martin' }, { screen_name: 'Ellie', age: 10 }, { screen_name: 'John', bio: 'Hello world' });
  ...
  `);
    const [forbes3, martin2, ellie3, john3] = await users(db).insertOrUpdate(['screen_name'], { screen_name: 'Forbes', age: 20 }, { screen_name: 'Martin' }, { screen_name: 'Ellie', age: 5 }, { screen_name: 'John', bio: 'Whatever world' });
...
test('create users in bulk', async () => {
    ...
    const records = await users(db)
        .find({ screen_name: (0, __1.anyOf)(names) })
...
test('query users in bulk', async () => {
    ...
    expect(await users(db)
        ...
        .andWhere({
        screen_name: (0, __1.anyOf)([`bulk_insert_name_3`, `bulk_insert_name_5`]),
    })
        .select(`screen_name`, `age`, `bio`)
        .orderByAsc(`screen_name`)
        .all()).toEqual([
...
test('delete users in bulk', async () => {
    ...
    expect(await users(db)
        ...
        .select(`screen_name`, `age`)
        .orderByAsc(`screen_name`)
        .all()).toEqual([{ screen_name: `bulk_insert_name_17`, age: 42 }]);
});
...
//# sourceMappingURL=bulk-insert.test.pg.js.map