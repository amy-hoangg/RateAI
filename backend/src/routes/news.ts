...
router.patch('/dislike/:id', async (req: Request, res: Response) => {
  const newsId = req.params.id;
  ...
  try {
    ...
    // Update the saves count for the News
    const updatedNews = await newsService.dislike(newsId, decodedToken.id);
    ...
    console.error("Error dislike news:", error);
    return res.status(500).json({ error: "An error occurred while dislike news" });
  }
});

router.patch('/removeDislike/:id', async (req: Request, res: Response) => {
  const newsId = req.params.id;
  ...
  try {
    ...
    // Update the saves count for the News
    const updatedNews = await newsService.removeDislike(newsId, decodedToken.id);
...
router.patch('/removeLike/:id', async (req: Request, res: Response) => {
  const newsId = req.params.id;
  ...
  try {
    ...
    // Update the saves count for the News
    const updatedNews = await newsService.removeLike(newsId, decodedToken.id);
